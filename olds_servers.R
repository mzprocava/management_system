...
# library(lares, warn.conflicts = FALSE) ### for translation of columns
...
############################################### REACTIVE DATASETS #############################
  ...
  output$payment_approvals <- DT::renderDataTable({
    DT  <- as.data.frame(approval_requests())
    DT[["Finance_approval"]]<-
      paste0('
             <div class="btn-group" role="group" aria-label="Approval">
                <button type="button" class="fa fa-edit approvals" style="color:blue" id=approvals_',1:nrow(DT),'></button>
             </div>')

    DT <- DT %>% select(Finance_approval, everything())

    names(DT) <- c('', 'PAAO', 'Despesa', 'Beneficiário', 'Submissão', 'Proponente', 'CED', 'Unidade', '#', 'Valor', 'Descrição', 'Moeda', 'Urgência', 'Comentários', 'Sectores', 'UGB', 'Tipologia', 'Sector Financeiro', 'Sector de Planificação', 'Sector de Contratação', 'Aprovação Global')

    datatable(DT, escape = FALSE,  rownames=F, extensions = c('Buttons'), options = list(dom = 'Bfrtip', buttons = c('pageLength', 'copy', 'csv', 'excel', 'pdf', 'print'),  pagelength = 10, lengthMenu = list(c(10, 25, 100, -1), c('10', '25', '100','All')),
                                                                                         columnDefs = list(list(targets = c(13:(ncol(DT)-1)), visible = FALSE))))%>%
      formatCurrency(c('Valor','#'), '') %>%
      formatStyle(c('Despesa', 'Comentários'), `text-align` = 'center')
  })
  ...
  output$execucao_componente <- DT::renderDataTable({
    ...
    component_quarters <- component_quarters %>% arrange(copy) %>% fselect(-copy, -component)
  ...
  output$execucao_financiador <- DT::renderDataTable({
    ...
    paid_financiers <- paid_financiers %>% group_by(funding, quarters) %>% summarise(soma_paga = sum(paid_ammount))  %>%
      ...
      adorn_totals("col")
    PDR_financiers <- cost_tabs_pdr %>% fsubset(class == "financiers") %>% select(funding = Expenditure, PDR = total_cost)
  ...
  #  #####################  FINANCE VALUE BOXES
   ...
   output$executado_rpsf  <- renderValueBox({
     valor_rpsf <- full_approved_payments() %>% fsubset(pdr_financiers_en %in% c("RPSF 2nd Allocation", "RPSF 1st Allocation"))
   ...
   output$comparticipacao_governo  <- renderValueBox({
     valor_governo <-full_approved_payments() %>% fsubset(pdr_financiers_en %in% c("Government"))
    ...
    output$components_quarters <- DT::renderDataTable({
      ...
      C1 <- completions %>% fsubset(componentnum_pt == "Componente 1") %>% adorn_totals("row", name = "Component 1") %>% arrange(components_pt)
      C2 <- completions %>% fsubset(componentnum_pt == "Componente 2") %>% adorn_totals("row", name = "Component 2")%>% arrange(components_pt)
      C3 <- completions %>% fsubset(componentnum_pt == "Componente 3") %>% adorn_totals("row", name = "Component 3")%>% arrange(components_pt)
      C4 <- completions %>% fsubset(componentnum_pt == "Componente 4") %>% adorn_totals("row", name = "Component 4")%>% arrange(components_pt)
      completions <- dplyr::bind_rows(C1, C2, C3, C4) %>% fsubset(!is.na(PDR))
      TT <- completions %>% filter(componentnum_pt=="-")
      TT <- add_row(TT, summarize(TT, across(where(is.numeric), ~ sum(., na.rm = T)))) %>% fsubset(is.na(components_pt))
      TT$components_pt[is.na(TT$components_pt)] <- "Total"
      completions <- dplyr::bind_rows(completions, TT) %>% mutate(awpb_pct = Total/AWPB*100, pdr_pct = Total/PDR*100) %>% fselect(-componentnum_pt)
   ...
   output$category_quarters <- DT::renderDataTable({
     ...
     pdr_categories <- costabs_summaries %>% fsubset(class == "categories") %>% fselect(component_nums, Description, PDR = total_cost)
   ...
   output$financiers_quarters <- DT::renderDataTable({
     ...
     pdr_categories <- costabs_summaries %>% fsubset(class == "financiers") %>% fselect(component_nums, Description, PDR = total_cost)
   ...
   #   payments_dataset <- payments_dataset %>% fselect(description, beneficiary, nuit, document, awpb_id, payment_details, units, quantity, price, paid_ammount,
   ...
   observeEvent(input$salvar_xls_ip, priority = 20, {
     ...
     payments_dataset <- payments_dataset %>% fselect(expense_description, detailed_en, e_sistafe_w_code, payment_beneficiary, 
                                                      ...
                                                      ifadrpsf1_pct, ifadrpsf2_pct, privateinkind_pct, privatemoney_pct, process_id,
  ...
  ################ INICIO DO PROBLEMA  ####################
   ...
   observeEvent(input$escrever_ip, {
     ...
     # payments_data <- payments_data %>% fsubset(tipos_de_despesas == tipo_despesa)
     ...
     showModal(modalDialog("DOCUMENTO SALDO NO 'MY DOCUMENTS!", easyClose = TRUE, footer = NULL, class = "success")) 
   ...
   output$component_contracts <- DT::renderDataTable({
     ...
     C1 <- contract_components %>% fsubset(componentnum_pt == "Componente 1") %>% adorn_totals("row") %>% mutate(components_pt = replace(components_pt, components_pt == "Total", "Componente 1")) %>% arrange(components_pt)
     C2 <- contract_components %>% fsubset(componentnum_pt == "Componente 2") %>% adorn_totals("row") %>% mutate(components_pt = replace(components_pt, components_pt == "Total", "Componente 2"))%>% arrange(components_pt)
     C3 <- contract_components %>% fsubset(componentnum_pt == "Componente 3") %>% adorn_totals("row") %>% mutate(components_pt = replace(components_pt, components_pt == "Total", "Componente 3"))%>% arrange(components_pt)
     C4 <- contract_components %>% fsubset(componentnum_pt == "Componente 4") %>% adorn_totals("row") %>% mutate(components_pt = replace(components_pt, components_pt == "Total", "Componente 4"))%>% arrange(components_pt)
     
     aggreement_components <- dplyr::bind_rows(C1, C2, C3, C4, totais, tt_pct)
     aggreement_components <- aggreement_components %>% fselect(-2) %>% adorn_totals("col")
   ...
   output$dossiers_iniciados  <- renderValueBox({
     dossiers_iniciados <- filtered_dossiers() %>% fsubset(procurement_stage != "Não iniciado")
   ...
   output$dossiers_contratados  <- renderValueBox({
     dossiers_contratados <- filtered_dossiers() %>% fsubset(procurement_stage %in% c("Contrato Assinado", "Contrato visado", "Contrato em Implementação", "Contrato encerrado"))
     total_dossiers <- n_distinct(filtered_dossiers()$activ_id)
   ...
   output$usd_latentes   <- renderValueBox({
     dossiers_latentes <- filtered_dossiers() %>% fsubset(procurement_stage == "Não iniciado")
   ...
   #  #####################  CONTRACT VALUE BOXES
    ...
    output$dossiers_methods <- DT::renderDataTable({
      ...
      ctots  <-   filtered_dossiers()  %>% group_by(ifadpp_sheet, simplexstage_en) %>% summarize(montante = n_distinct(activ_id)) %>%
        pivot_wider(names_from = simplexstage_en, values_from = montante) %>% adorn_totals("row") %>% fsubset(ifadpp_sheet== "Total")
    ...
    output$dossiers_components <- DT::renderDataTable({
      ...
      ctots  <-   filtered_dossiers()  %>% group_by(componentnum_pt, simplexstage_en) %>% summarize(montante = n_distinct(activ_id)) %>%
        pivot_wider(names_from = simplexstage_en, values_from = montante) %>% adorn_totals("row") %>% fsubset(componentnum_pt== "Total")
                              ...
                              div(
                                fluidPage(
                                  fluidRow(
                                    ...
                                    # radioButtons('via_de_entrada', 'Via de Entrada', choices = c('Ficheiro Excel', 'Formulário Electrónico' = "Electronic form"), selected = '', inline = TRUE, width = "100%"),
                                    
                                    # radioGroupButtons("via_de_entrada", "Via de Entrada", choices = c('Ficheiro Excel', 'Formulário Electrónico' = "Electronic form"), status = "primary", width = "470px"),
                                    radioGroupButtons("via_de_entrada", "Via de Entrada", choices = c('Ficheiro Excel', 'Formulário Electrónico' = "Electronic form"), status = "primary", checkIcon = list(yes = icon("ok", lib = "glyphicon"), no = icon("remove",lib = "glyphicon")), width = "470px"),
                                    
                                    
                                    
                                    
                                    div(style = "display: inline-block;", 
                                        selectizeInput('awpb_ids','Codigo no PAAO', choices = codes, selected = 0, width = "140px")),
                                    
                                    div(style = "display: inline-block;", 
                                        selectizeInput('cost_tabs_fk','COST TABS', choices = costab_code, selected = 0, width = "130px")),
                                    ...
                                    radioGroupButtons("expense_urgencia_processo", "Urgência do processo", choices = c('NORMAL', 'URGENTE', 'MUITO URGENTE'), status = "primary", checkIcon = list(yes = icon("ok", lib = "glyphicon"), no = icon("remove",lib = "glyphicon")), width = "470px"),
                                    
                                    # radioGroupButtons("expense_urgencia_processo", "Urgência do processo", choices = c('MUITO URGENTE', 'URGENTE', 'NORMAL'), status = "primary", width = "300px"),
                                    
                                    # radioButtons('expense_urgencia_processo','Urgência do processo', choices = c('MUITO URGENTE', 'URGENTE', 'NORMAL'), selected = 'NORMAL', inline = TRUE),
                                    
                                    br(),
                                    div(style = "height:30px"),
                                    selectizeInput('expense_currency','Moeda do Processo', choices = c('MZN', 'USD', 'EUR', 'ZAR', 'GBP'), selected = 'Todos', multiple = FALSE, width = "100%"),
                                    ...
                                    conditionalPanel(condition = "input.via_de_entrada == 'Electronic form'",
                                                     ...
                                                     # radioGroupButtons("is_contract", "É Contrato?", choices = c('SIM', 'NÃO'), status = "primary", width = "300px"),
                                                     
                                                     
                                                     radioGroupButtons(inputId = "is_contract", label = "É Contrato?", choices = c('SIM', 'NÃO'), status = "primary", checkIcon = list(yes = icon("ok", lib = "glyphicon"), no = icon("remove",lib = "glyphicon")), width = "100%")),
                                    ...
                                    div(style = "display: inline-block;", conditionalPanel(condition = "input.via_de_entrada == 'Ficheiro Excel' && input.financier_paying != 'FIDA' && input.financier_paying != 'COVID'",
                                                                                           DT::dataTableOutput("payments_xls_comparticipacao"))),
                                    
                                    div(style = "display: inline-block;", conditionalPanel(condition = "input.via_de_entrada == 'Ficheiro Excel' && input.financier_paying != 'FIDA' && input.financier_paying != 'COVID'",
                                                                                           actionButton("payments_xls_comparticipacao", "COMETER DESPESA", style = "color: white;  background-color: blue; ", icon=icon("plus"))))
                                  ),
                                   ...
                                   div(id=("new_awpb_update_form"),
                                       ...
                                       fluidPage(
                                         fluidRow(
                                           ...
                                           div(style = "display: inline-block;", conditionalPanel(condition = "input.unidade_de_management == 'UPGPN'", 
                                                                                                  selectInput("situacao_upgpn", labelMandatory("Situação"), multiple = FALSE, choices = awpb_situacoes, width = "350px"))),

                                           div(style = "display: inline-block;", conditionalPanel(condition = "input.unidade_de_management == 'UNGP'", 
                                                                                                  selectInput("situacao_ungp", labelMandatory("Situação"), multiple = FALSE, choices = awpb_situacoes, width = "350px"))),
       
                                           div(style = "display: inline-block;", conditionalPanel(condition = "input.unidade_de_management == 'URGPS'", 
                                                                                                  selectInput("situacao_urgps", labelMandatory("Situação"), multiple = FALSE, choices = awpb_situacoes, width = "350px"))),
                                           
                                           conditionalPanel(condition = "input.update_planned_targets != 'REVER METAS MENSAIS'", 
                                           ...
                                           conditionalPanel(condition = "input.idioma_update == 'PT' && input.unidade_de_management == 'UPGPN'  && input.update_planned_targets != 'REVER METAS MENSAIS'", 
                                           textAreaInput("comments_niassa_pt", "Descrição do Ponto de Situação", placeholder = "", height = 100, width = "470px")),
                                           conditionalPanel(condition = "input.idioma_update == 'EN' && input.unidade_de_management == 'UPGPN'  && input.update_planned_targets != 'REVER METAS MENSAIS'", 
                                           textAreaInput("comments_niassa_en", "Detailed Status Description", placeholder = "", height = 100, width = "470px")),
                                           
                                           conditionalPanel(condition = "input.idioma_update == 'PT' && input.unidade_de_management == 'URGPS' && input.update_planned_targets != 'REVER METAS MENSAIS'", 
                                           textAreaInput("comments_gaza_pt", "Descrição do Ponto de Situação", placeholder = "", height = 100, width = "470px")),
                                           conditionalPanel(condition = "input.idioma_update == 'EN' && input.unidade_de_management == 'URGPS' && input.update_planned_targets != 'REVER METAS MENSAIS'", 
                                           textAreaInput("comments_gaza_en", "Detailed Status Description", placeholder = "", height = 100, width = "470px")),
                                           
                                           conditionalPanel(condition = "input.idioma_update == 'PT' && input.unidade_de_management == 'UNGP' && input.update_planned_targets != 'REVER METAS MENSAIS'", 
                                           textAreaInput("comments_national_pt", "Descrição do Ponto de Situação", placeholder = "", height = 100, width = "470px")),
                                           conditionalPanel(condition = "input.idioma_update == 'EN' && input.unidade_de_management == 'UNGP' && input.update_planned_targets != 'REVER METAS MENSAIS'", 
                                           ...
                                           conditionalPanel(condition = "input.unidade_de_management == 'UNGP' && input.update_planned_targets != 'REVER METAS MENSAIS'", 
                                           ...
                                           conditionalPanel(condition = "input.unidade_de_management == 'URGPS' && input.update_planned_targets != 'REVER METAS MENSAIS'", 
                                           ...
                                           conditionalPanel(condition = "input.unidade_de_management == 'UPGPN'  && input.update_planned_targets != 'REVER METAS MENSAIS'", 
                                           ...
                                           radioGroupButtons("add_distict_targets", "", choices = c("NÃO EXISTEM DADOS REAIS", "EXISTEM DADOS REAIS"), status = "warning", checkIcon = list(yes = icon("ok", lib = "glyphicon"), no = icon("remove", lib = "glyphicon"))),
                                           
                                           conditionalPanel(condition = "input.add_distict_targets == 'EXISTEM DADOS REAIS' && input.update_planned_targets != 'REVER METAS MENSAIS'",
                                                            actionButton("optimize","REALIZAÇÃO POR DISTRITO OU UNIDADE", class = "run-button", style="color: #fff; background-color: #0020C2; border-color: #0020C2; height: 30px; width:465px;"),
                                                            
                                                            div(style = "display: inline-block;", selectizeInput("distrito_beneficiario", labelMandatory("Distrito ou Unidade"), multiple = FALSE, choices = c("", distritos_procava), selected = "", width = "150px")),
                                                            div(style = "display: inline-block;", selectizeInput("district_status", labelMandatory("Situação no Distrito"), multiple = FALSE, choices = c("", awpb_situacoes), selected = "",  width = "310px")),
                                                            selectInput("target_units", "Unidades de Medida", multiple = FALSE, choices = c('', as.character(unique(awpb_updated$unidades))), width = "470px"),
                                                            div(style = "display: inline-block;", autonumericInput("in_progress","Em curso", value = 0, align = "left", currencySymbol = '', currencySymbolPlacement = "p", decimalCharacter = ",", digitGroupSeparator = ".", formulaMode = TRUE, width = "170px")),
                                                            div(style = "display: inline-block;", autonumericInput("value_achieved","Já Realizados", value = 0, align = "left", currencySymbol = '', currencySymbolPlacement = "p", decimalCharacter = ",", digitGroupSeparator = ".", formulaMode = TRUE, width = "170px")),
                                                            div(style = "display: inline-block;", dateInput('updated_day',label='Realização', width = "120px")),
                                                            br(),

                                                            actionButton("adicionar_realizacao", "Cometer", style = "color: white;  background-color: blue; ", icon = icon("plus")),
                                                            br(),
                                                            actionButton("optimize","", class = "run-button", style="color: #fff; background-color: #0020C2; border-color: #0020C2; height: 10px; width:465px;")),
                                           ...
                                           radioGroupButtons("update_planned_targets", "", choices = c("REVER METAS MENSAIS", "MANTER METAS MENSAIS"), selected = "MANTER METAS MENSAIS", status = "danger", checkIcon = list(yes = icon("ok", lib = "glyphicon"), no = icon("remove", lib = "glyphicon"))),
                                           # prettySwitch(inputId = "update_planned_targets", label = "REVER METAS MENSAIS",  status = "success", value = FALSE, fill = TRUE),
                                           
                                          ###########################   REVISÃO DE METAS MENSAIS #############  = ELABORAÇÃO DE IFR  ####################
                                         
                                           conditionalPanel(condition = "input.update_planned_targets == 'REVER METAS MENSAIS' && input.unidade_de_management == 'UNGP'",
                                                            # actionButton("optimize","REALIZAÇÃO POR DISTRITO OU UNIDADE", class = "run-button", style="color: #fff; background-color: #321414; border-color: #321414; height: 30px; width:465px;"),
                                                            actionButton("optimize_2","REPROGRAMAÇÃO DE METAS DAS ACTIVIDADES", class = "run-button", style="color: #fff; background-color: #C24641; border-color: #C24641; height: 30px; width:465px;")),
                                                            # selectInput("target_units", "Unidades de Medida", multiple = FALSE, choices = c('', as.character(unique(awpb_updated$unidades))), width = "470px"),
                                                            # div(style = "display: inline-block;", autonumericInput("orcamento_revisto_ungp","Proposta de Revisão Orçamental (US$)", value = 0, align = "right", currencySymbol = '', currencySymbolPlacement = "p", decimalCharacter = ",", digitGroupSeparator = ".", formulaMode = TRUE, width = "260px")),
                                                            # div(style = "display: inline-block;", shiny::selectizeInput(inputId = "unidades_medida_ungp", 'Unidade', choices = units_indicators, multiple = FALSE, width = "200px")),
                                          ...
                                          conditionalPanel(condition = "input.update_planned_targets == 'REVER METAS MENSAIS' && input.unidade_de_management == 'UNGP'",
                                                            ...
                                                            actionButton("salvar_metas_alteradas_ungp", "Cometer", style = "color: white;  background-color: #CD5C5C; ", icon = icon("plus")),
                                                            br(),
                                           ...
                                           conditionalPanel(condition = "input.update_planned_targets == 'REVER METAS MENSAIS' && input.unidade_de_management == 'URGPS'",
                                                            actionButton("optimize_2","REPROGRAMAÇÃO DE METAS DAS ACTIVIDADES", class = "run-button", style="color: #fff; background-color: #C24641; border-color: #C24641; height: 30px; width:465px;")),
                                                            
                                                            # div(style = "display: inline-block;", autonumericInput("orcamento_revisto_urgps","Proposta de Revisão Orçamental (US$)", value = 0, align = "right", currencySymbol = '', currencySymbolPlacement = "p", decimalCharacter = ",", digitGroupSeparator = ".", formulaMode = TRUE, width = "260px")),
                                                            # div(style = "display: inline-block;", shiny::selectizeInput(inputId = "unidades_medida_urgps", 'Unidade', choices = units_indicators, multiple = FALSE, width = "200px")),
                                                            # 
                                          conditionalPanel(condition = "input.update_planned_targets == 'REVER METAS MENSAIS' && input.unidade_de_management == 'URGPS'",div(style = "display: inline-block;", autonumericInput("orcamento_revisto_urgps","Orçamento Revisto (US$)", value = 0, align = "right", currencySymbol = '', currencySymbolPlacement = "p", decimalCharacter = ",", digitGroupSeparator = ".", formulaMode = TRUE, width = "160px"))),
                                          conditionalPanel(condition = "input.update_planned_targets == 'REVER METAS MENSAIS' && input.unidade_de_management == 'URGPS'",div(style = "display: inline-block;", autonumericInput("paao_urgps","Meta", value = 0, align = "right", currencySymbol = '', currencySymbolPlacement = "p", decimalCharacter = ",", digitGroupSeparator = ".", formulaMode = TRUE, width = "135px"))),
                                          conditionalPanel(condition = "input.update_planned_targets == 'REVER METAS MENSAIS' && input.unidade_de_management == 'URGPS'",div(style = "display: inline-block;", shiny::selectizeInput(inputId = "unidades_medida_urgps", 'Unidade', choices = units_indicators, multiple = FALSE, width = "160px"))),
                                          conditionalPanel(condition = "input.update_planned_targets == 'REVER METAS MENSAIS' && input.unidade_de_management == 'URGPS'",                  
                                                            ...
                                                            # div(style = "display: inline-block;", autonumericInput("orcamento_revisto_urgps","Proposta de Revisão Orçamental (US$)", value = 0, align = "right", currencySymbol = '', currencySymbolPlacement = "p", decimalCharacter = ",", digitGroupSeparator = ".", formulaMode = TRUE, width = "370px")),
                                                            br(),
                                                            
                                                            actionButton("salvar_metas_alteradas_urgps", "Cometer", style = "color: white;  background-color: #CD5C5C; ", icon = icon("plus")),
                                                            br(),
                                                            actionButton("optimize_3","", class = "run-button", style="color: #fff; background-color: #C24641; border-color: #C24641; height: 10px; width:465px;")),
                                           
                                           conditionalPanel(condition = "input.update_planned_targets == 'REVER METAS MENSAIS' && input.unidade_de_management == 'UPGPN'",
                                                            actionButton("optimize_2","REPROGRAMAÇÃO DE METAS DAS ACTIVIDADES", class = "run-button", style="color: #fff; background-color: #C24641; border-color: #C24641; height: 30px; width:465px;")),
                                          ...
                                          conditionalPanel(condition = "input.update_planned_targets == 'REVER METAS MENSAIS' && input.unidade_de_management == 'UPGPN'",div(style = "display: inline-block;", autonumericInput("paao_upgpn","Meta", value = 0, align = "right", currencySymbol = '', currencySymbolPlacement = "p", decimalCharacter = ",", digitGroupSeparator = ".", formulaMode = TRUE, width = "135px"))),
                                          conditionalPanel(condition = "input.update_planned_targets == 'REVER METAS MENSAIS' && input.unidade_de_management == 'UPGPN'",div(style = "display: inline-block;", shiny::selectizeInput(inputId = "unidades_medida_upgpn", 'Unidade', choices = units_indicators, multiple = FALSE, width = "160px"))),
                                                            ...
                                                            actionButton("salvar_metas_alteradas_upgpn", "Cometer", style = "color: white;  background-color: #CD5C5C; ", icon = icon("plus")),
                                                            br(),
                                                            actionButton("optimize_3","", class = "run-button", style="color: #fff; background-color: #C24641; border-color: #C24641; height: 10px; width:465px;")),
